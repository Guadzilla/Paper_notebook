...
def Relation(user_train, usernum, maxlen, time_span):   # 返回所有user的relation矩阵
    data_train = dict()
    for user in tqdm(range(1, usernum + 1), desc='Preparing relation matrix'):
        time_seq = np.zeros([maxlen], dtype=np.int32)
        idx = maxlen - 1
        for i in reversed(user_train[user][:-1]):   # 把处理完的时间取到time_seq里
            time_seq[idx] = i[1]
            idx -= 1
            if idx == -1: break
...
def sample_function(user_train, usernum, itemnum, batch_size, maxlen, relation_matrix, result_queue, SEED):
    def sample(user):
        ...
        seq = np.zeros([maxlen], dtype=np.int32)        # 训练序列
        time_seq = np.zeros([maxlen], dtype=np.int32)   # 时间序列
        pos = np.zeros([maxlen], dtype=np.int32)        # 正例序列
        neg = np.zeros([maxlen], dtype=np.int32)        # 负例序列
        nxt = user_train[user][-1][0]

        idx = maxlen - 1
        ts = set(map(lambda x: x[0], user_train[user]))
        for i in reversed(user_train[user][:-1]):   # i[0]是物品，i[1]是对应时间
            seq[idx] = i[0]
            time_seq[idx] = i[1]
            pos[idx] = nxt
...
class WarpSampler(object):  # 和SASRec几乎一样，区别就在多了时间特征，需要多返回一个time_seq
...
def timeSlice(time_set):    # 返回“key=时间戳，value=减去最小时间的时间片”的字典time_map
...
def cleanAndsort(User, time_map):
    ...
    user_map = dict()
    item_map = dict()
    for u, user in enumerate(user_set):     # 对user和item重新编号，从1开始
        user_map[user] = u + 1
    for i, item in enumerate(item_set):
        item_map[item] = i + 1

    for user, items in User_filted.items():     # 每个user的序列按时间顺序重排序
        User_filted[user] = sorted(items, key=lambda x: x[1])

    User_res = dict()
    for user, items in User_filted.items():     # User_res 是 key: user, value:[I,T]     其中user,I,T都是重新编号的
        User_res[user_map[user]] = list(map(lambda x: [item_map[x[0]], time_map[x[1]]], items))

    time_max = set()
    for user, items in User_res.items():        # 对每条序列的时间做缩放操作
        time_list = list(map(lambda x: x[1], items))
        time_diff = set()   # time_diff:用来记录时间间隔
        ...
        if len(time_diff) == 0:
            ...
            time_scale = min(time_diff)     # 最小的时间间隔作为缩放系数
        time_min = min(time_list)
        # scaled time intervals
        User_res[user] = list(map(lambda x: [x[0], int(round((x[1] - time_min) / time_scale) + 1)], items))  # 缩放公式
        time_max.add(max(set(map(lambda x: x[1], User_res[user]))))     # 每条序列的时间最大值
...
def data_partition(fname):
    ...
    for line in f:
        ...
        if user_count[u] < 5 or item_count[i] < 5:  # hard-coded 长度小于5的序列，和出现次数小于5的物品，统统不计
            continue
        time_set.add(timestamp)     # 时间戳集合,set对象的add()方法
        User[u].append([i, timestamp])  # User[i] = [[item1,timestamp1],[item2,timestamp2],...]
    f.close()
...
def evaluate(model, dataset, args):
    ...
    for u in users:
        ...
        predictions = predictions[0]    # (1,101) --> 101

        rank = predictions.argsort().argsort()[0].item()    # 两次argsort得到每个位置的排名
